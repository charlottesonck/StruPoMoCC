...
realtype TIN=RCONST(0.5); /*agestep*/
...
alglib::real_2d_array JacobiaanGS0(realtype Sa, realtype S0a, clist coh, int out){ 
...
alglib::real_2d_array JacobiaanGD(realtype Sa, realtype Da, clist coh, int out){ 
	/* calculates the Jacobian of the map G := mapM_1 - identity (has to be zero for a fixed point)
...
alglib::real_2d_array JacobiaanM1(realtype Sa, clist coh, int out){ 
...
int contfpS0(realtype &Sa, realtype &S0a, clist &coh, realtype step_start, realtype step_m, realtype step_M, int Newtonmax, realtype FunTolerance,realtype VarTolerance,int dir, int a, int out){ 
	...
	JG = JacobiaanGS0(Sa,S0a,coh,out);
	myfile << " Jacobiaan:" <<endl;
	...
	J1 = JacobiaanGS0(Sp,S0p,cohp,out);
	myfile << " Jacobiaan J1:" << endl;
